...
class CustomerController extends Controller
{
    ...
    {
        $this->middleware('customer', ['except' => ['register', 'store', 'verify', 'resendotp', 'account_verify', 'login', 'signin', 'logout', 'checkout', 'forgot_password', 'forgot_verify', 'forgot_reset', 'forgot_store', 'forgot_resend', 'order_save', 'order_success', 'order_track', 'order_track_result', 'incomplete_order_save']]);
    }
    ...
    //     if ($request->area && $request->area !== '' && $request->area !== 'ডেলিভারি এরিয়া নিবার্চন করুন *') {
    //         $address = ($address ? $address . ' - ' : '') . $request->area;
    //     }
    ...
    public function incomplete_order_save(Request $request)
    {
        // ফোন নাম্বার যদি থাকে তবেই ভ্যালিডেশন ধরবে
        $request->validate([
            'phone' => 'required',
        ]);
